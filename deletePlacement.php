...
function delete(){
    ...
    try {
        ...
        $query = $db->prepare("DELETE FROM placement WHERE id=?");
        $query->execute([$_POST['placement_id']]);
    ...
    if (isset($_SESSION['access_token']) && $_SESSION['access_token']) {
        ...
        $email = $_SESSION['email'];
        putInHistory($email, 0, "Deleted placement", "Google account");
        ...
        $userid = $userid[0]->id;
        putInHistory($id, $userid, "Deleted placement", "User account");
    }
    echo '<script>alert("Deleted placement");
    window.location.href="admin.php";</script>';
    //header('Location: admin.php');
    exit;
}